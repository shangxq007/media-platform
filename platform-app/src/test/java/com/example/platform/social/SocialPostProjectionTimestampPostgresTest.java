...
class SocialPostProjectionTimestampPostgresTest extends PostgresTestContainerSupport {
    ...
    @Test void allEntityBindingsAndRawCounterexampleInDifferentTimezones() {
        inZone(repo -> {
            ...
            repo.updatePublishResult("entity","external","url",PostStatus.PUBLISHED,T.plusSeconds(3),T.plusSeconds(4));
            repo.updateFailure("entity","code","message",PostStatus.FAILED,T.plusSeconds(5),2,T.plusSeconds(6));
            saved=repo.findById("entity").orElseThrow();
            assertThat(saved.publishedAt()).isEqualTo(T.plusSeconds(3));
            assertThat(saved.failedAt()).isEqualTo(T.plusSeconds(5));
            assertThat(saved.updatedAt()).isEqualTo(T.plusSeconds(6));
