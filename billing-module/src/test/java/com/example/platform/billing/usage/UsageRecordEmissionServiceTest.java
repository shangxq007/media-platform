...
@SpringBootTest(
        classes = UsageRecordEmissionServiceTest.TestConfig.class,
        properties = {
            "spring.flyway.enabled=false",
            "spring.sql.init.mode=never"
        })
class UsageRecordEmissionServiceTest extends PostgresTestContainerSupport {
    ...
    @Autowired
    private UsageRecordEmissionPort emissionPort;
    ...
    @Test
    void emit_persistsUsageAndOutboxEventViaPort() {
        UsageRecord saved = emissionPort.emit(record("tenant-1", "idem-port"));
    ...
    static class TestConfig {
        ...
        @Bean
        public UsageRecordEmissionService usageRecordEmissionService(
                UsageOutboxEventPublisher usageOutboxEventPublisher) {
            return new UsageRecordEmissionService(usageOutboxEventPublisher);
        }
    }
}
