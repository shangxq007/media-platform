...
## Phase 2b Agent Permission Model (2026-07-01)
...
| OpenCode | 2 | Feature Branch Pusher (assigned TASK_IDs) |
| Codex | 2 | Feature Branch Pusher (assigned TASK_IDs) |
| Kilo Code | 2 | Feature Branch Pusher (assigned TASK_IDs) |
| Claude Code | 2 | Feature Branch Pusher + Reviewer |
| Aider | 1→2 | Level 1 default, Level 2 for review-fix |
| OpenHands | 1 | Inactive unless explicitly assigned |
| Human | 5 | Final Arbiter |

### Allowed Actions (Level 2)

- Create task branch
- Local commit
- Run tests
- Fix own branch
- Push feature branch (assigned TASK_ID only)
- Create draft PR (Hermes Level 3)
- Generate review packet

### Forbidden Actions

- Push main (Human only)
- Force push (Never)
- Merge PR (Human only)
- Auto approve (Never)
- Production deploy (Human only)
- Production secrets (Except authorized)
- Flyway V1 modification (Never)
- Spring AI activation (Never)
- Artifact DAG runtime (Never)
- OpenCue-as-Provider (Never)
- Remotion production dispatch (Never)

### Stop Conditions

See `policies/stop-conditions.md` for 12 stop conditions.

### Review Packet Requirement

Every task completion requires a Review Packet generated by Hermes.

### Draft PR Policy

- Draft PR allowed for assigned TASK_ID branches
- PR title must include TASK_ID
- PR body must include review packet link
- PR stays draft until milestone review
- No auto merge

### ARCH.1 Constraints

1. CT.0 must remain typed intent slice
2. VS.0 must use FFmpeg/libass only
3. No schema changes unless necessary
4. All subtasks respect stop-conditions.md
5. Review packets per task
6. Claude Code architecture review at milestone
7. Human reviews before merge

## Frontend Architecture Constraints (FRONTEND-ARCH.0)

### Core Decision

Future frontend work is **contract-first** and should not be treated as a migration from the old UI.

### Styling Technology

- Do not introduce frontend-wide styling libraries into production code without a scoped POC and architecture approval.
- StyleX and Astryx are candidates for evaluation, not adopted globally.
- Do not bind render output style semantics to Web UI styling implementation.

### Editor Components

- Editor core components (timeline, canvas, caption editor, frame-accurate interactions) should remain **platform-owned**.
- Do not use generic component libraries for editor core interactions.
- Radix primitives may be used for dialogs, popovers, selects, tabs, tooltips, menus.

### Render Boundary

- Remotion preview should consume **render-facing schemas**, not UI CSS.
- Video render styles use **platform-owned schemas** (CaptionStyleSchema, TextStyleToken, FontManifest).
- StyleX/Astryx/Tailwind are for Web UI only, not video render style DSL.

### Implementation Gate

- Do not start frontend implementation before backend contracts are stable.
- Key contracts: RenderJob API, Timeline model, Caption template schema, Font manifest, Asset contract.

### Documentation

- See `docs/frontend/new-frontend-architecture.md` for architecture plan
- See `docs/frontend/styling-technology-decision.md` for styling evaluation
- See `docs/frontend/new-frontend-roadmap.md` for roadmap
- See `docs/frontend/editor-architecture.md` for editor component ownership
- See `docs/frontend/render-preview-contract.md` for Remotion boundary
- See `docs/frontend/design-token-and-video-style-boundary.md` for style layers
