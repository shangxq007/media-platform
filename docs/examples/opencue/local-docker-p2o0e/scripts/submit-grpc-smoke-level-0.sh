#!/usr/bin/env bash
# P2O.0e — Submit Smoke Level 0: Shared Path Probe via Cuebot gRPC
# Submits a true OpenCue job through grpcurl using LaunchSpecAndWait.
# Facility=local, show=testing, shot=p2o0e-probe, user=operator.
...
JOB_NAME="p2o0e-smoke-level-0"
SHOW="testing"
SHOT="p2o0e-probe"
FACILITY="local"
USER="operator"

echo "[P2O.0e] Smoke Level 0 — Shared Path Probe (gRPC Submission)"
...
# Build CJSL XML spec for shared path probe
CJSL_SPEC='<?xml version="1.0" encoding="UTF-8"?>
<spec>
  <job name="'"${JOB_NAME}"'" shot="'"${SHOT}"'" show="'"${SHOW}"'" user="'"${USER}"'">
    <layer name="probe" type="RENDER">
      <cmd>
        <arg>mkdir -p /mnt/opencue-shared/media-platform-smoke/preview/p2o0e/smoke-level-0 &amp;&amp; echo p2o0e-true-submission-probe &gt; /mnt/opencue-shared/media-platform-smoke/preview/p2o0e/smoke-level-0/shared-path-probe.txt</arg>
      </cmd>
      <range>1-1</range>
      <chunk>1</chunk>
      <memory>100</memory>
...
# Extract job name from response for polling
...
while [ "${ELAPSED}" -lt "${MAX_WAIT}" ]; do
    # Poll job status from PostgreSQL
...
# Write job summary from database
JOB_SUMMARY_FILE="${PREVIEW_ROOT}/smoke-level-0/opencue-job-summary.txt"
...
echo "[P2O.0e] Job summary: ${JOB_SUMMARY_FILE}"
echo "[P2O.0e] Smoke Level 0 — PASS"
