#!/usr/bin/env bash
# P2O.0e — Submit Smoke Level 1: FFmpeg Probe via Cuebot gRPC
# Submits a true OpenCue job that generates a 1-second blue video with ffmpeg.
...
JOB_NAME="p2o0e-smoke-level-1"
SHOW="testing"
SHOT="p2o0e-probe"
FACILITY="local"
USER="operator"
OUTPUT_DIR="/mnt/opencue-shared/media-platform-smoke/preview/p2o0e/smoke-level-1"

echo "[P2O.0e] Smoke Level 1 — FFmpeg Probe (gRPC Submission)"
...
# Write job summary
JOB_SUMMARY_FILE="${PREVIEW_ROOT}/smoke-level-1/opencue-job-summary.txt"
...
if [ -f "${PREVIEW_ROOT}/smoke-level-1/output.mp4" ]; then
    echo "[P2O.0e] Output: ${PREVIEW_ROOT}/smoke-level-1/output.mp4"
    ls -la "${PREVIEW_ROOT}/smoke-level-1/output.mp4"
    echo "[P2O.0e] Smoke Level 1 — PASS"
else
    echo "[P2O.0e] WARN: Output not yet on host. Run copy-preview-artifacts.sh to retrieve from Docker volume."
    echo "[P2O.0e] Smoke Level 1 — SUBMITTED (output pending copy)"
fi
