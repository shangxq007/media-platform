...
| Blocking/nonblocking | Basic authenticated browsing is supported by the 2026-09-13 Identity/Render read correction below. General Project Open and richer action/progress/Artifact projections remain separate gaps; historical scope findings are retained with their baseline. |
...
Historical assessment baseline: `6aca5112bb3c5e68ac7b11e8b52a02bbafdd3401`. The accepted backend read correction below supersedes these blockers only for its bounded read path. This source assessment supersedes the earlier unconditional “nonblocking basic list/detail” claim. It does not change a backend contract or authorize backend implementation. The frontend Render slice stops at **CORE_PATH_BLOCKED**; no fixture screen or alternate feature is substituted.
...
#### Authenticated Project / Render read correction (2026-09-13)

The existing `TenantProjectService` now implements Identity's published `ProjectReadQuery`; no parallel Project store, ACL, tenant resolver or endpoint was added. `GET /api/identity/tenants/{tenantId}/projects` resolves the canonical actor, requires present/equal ambient, actor and requested tenant, and returns only Projects permitted by the existing `READ` action. `GET /api/identity/projects/{projectId}` resolves within the authenticated ambient tenant, checks Project `READ` again, and queries Identity's repository by exact tenant and Project ID. The existing `ProjectResponse` provides `id`, `tenantId`, `name`, `description`, `status`, `createdAt`. Permission administration remains the existing Identity Role/Permission model; JWT role strings alone do not grant resource access.

This is a **Render-specific usable discovery/resolution path**, not proof of a Workspace→Project relation absent from the current Project schema. General FB-GAP-001 / `ProjectContextProvider` / Project Open remain unresolved and must not be changed to RESOLVED by the frontend merely from route metadata or Dashboard recency. The frontend may obtain its current tenant from the existing authenticated context/dashboard, use the authorized Identity Project result as the explicit Render read scope, and keep the general Project Open action disabled.

The existing Render list/detail URLs now enforce current actor/tenant consistency, Identity Project `READ` and ownership, and `READ` on the `RENDER_JOB` resource. Reads execute the original exact tenant/project/job repository predicates. A previously returned Project DTO is not a credential: subsequent requests re-evaluate permissions. Legacy public `RenderJobService.list/getById` consumers also use authorized Project scope; the no-tenant `listAll` fallback is removed. Existing mutation hydration remains private and command semantics are not expanded.

Successful Render responses retain exactly `id/projectId/timelineSnapshotId/profile/status` and the nine existing statuses; no progress, ETA, Artifact, timestamp, pagination or provider state was added. Missing authentication (including missing JWT tenant claim) yields 401, inconsistent tenant or denied Project/job permission yields 403 through existing denial handling, and an authorized absent/mismatched resource yields 404. Discovery returns an authorized subset (possibly empty); an authorized Project with no jobs returns 200 `[]`, distinct from a denied Render list. An unknown Project without a grant may be denied before existence is checked; no existence disclosure is promised.

Readiness: **basic authenticated Render browsing prerequisites PASS locally**. `RenderReadHttpTest` exercises the actual server on a random port with real JWT verification, canonical resolver, RBAC rows and Flyway V1 PostgreSQL. It covers discovery/resolution, five-field list/detail, same-tenant denial, missing/mismatched context, cross-tenant/project jobs, empty results, owner absence, and revocation with an unchanged JWT/earlier resolved context. Synthetic terminal rows are test data; no job submission, cancellation, business retry or provider execution was invoked. Independent unit negatives verify job-specific decision enforcement and missing ambient context. Validation is backend-only; frontend, production OIDC, provider execution and general Project Open are NOT_RUN/not established.

Frontend resume contract: use the accepted local main SHA pinned in `/home/user/Documents/workspace/audit-runs/EP05_EP08_LOCAL_BATCH_20260912/HANDOFF.md`; call the above Identity endpoints, then `/api/tenants/{tenantId}/projects/{projectId}/render-jobs` and `/{jobId}` with current authentication. Preserve real HTTP denial statuses and scope/session cancellation; retire the old `/api/v1` prefix, guessed `recentProjects[0]` binding and unscoped polling when implementing the frontend. No alternative Product-derived Timeline result or worker queue is an inventory substitute. Run `bash scripts/test-authority-modules.sh render-read` for the accepted backend contract. Full FB-GAP-005 remains open for richer projections/actions; only its basic read authorization blocker is closed by this slice.
...
The corrected backend source now defines one narrower canonical read: `GET /api/social/platforms?projectId`, `GET /api/social/posts?projectId&connectedAccountId&bindingVersion&start&end&limit`, and `GET /api/social/posts/{id}?projectId&connectedAccountId&bindingVersion`. The account query returns only active actor-owned accounts with an exact current-version Project post binding. The ranged list is constrained by tenant, canonical actor, Project, exact account ID/current binding version and platform consistency; unbound or stale-binding rows are excluded. Its range is `start <= scheduledAt < end`, its coverage is always `BOUNDED_PARTIAL`, and `scheduledAt` means planned publish time only. The detail route applies the same scope predicates and is the only way this contract can return a record without a plan time.

The frontend ordinary Publication route now consumes only that source through the existing authenticated transport. It first selects from the secured Project account result, then sends the exact selected account and UTC half-open month to the ranged list; it sends the current `bindingVersion` and does not send legacy `page/size`, status or provider parameters. Opening a row performs the corrected scoped detail read. Strict response parsing rejects unknown legacy/status/provider fields and incoherent optional content/time pairs; no fallback protocol or fixture is selected after an unavailable/error response. The explicit fixture provider remains verification injection only.

Successful DTOs carry endpoint decision `AUTHORIZED_LOCAL_PROJECT_ACCOUNT_READ` while the absent global five-factor catalog remains `UNKNOWN_FAIL_CLOSED`; the frontend does not mint a `SERVER` EffectiveAccess entry from that endpoint receipt. Content or planned time may be `NOT_PROVIDED`. Artifact references are supplied only with `artifactRelationState=AVAILABLE`; state-only RESTRICTED and NOT_PROVIDED remain distinct. Attempt, provider outcome/status/diagnostic, retry and actual timestamp facts are not in the contract and remain `NOT_PROVIDED / unknown` rather than an invented graph or authoritative empty relation. Empty ranged results never claim complete Project history, and the UI explicitly states that unscheduled records are outside every ranged list.

The original no-socket preparation established frontend-to-source implementation only; it did not prove live authenticated HTTP/security execution or backend build closure. Subsequent accepted V11/backend records and the task-owned 2026-09-12 read verification below are separate evidence. No social mutation, provider call, Artifact hydration, scheduler, analytics or public packaging is implied by this read-only slice.
...
#### Publication browsing improvement — 2026-09-12 (frontend only)

Current source authority is `PublicationAccountResponse` / `PublicationPostResponse` / `PublicationPostListResponse` in Social, consumed by the strict `PublicationReadAPI` and `platformClient.publication`. Read parameters include the exact positive account `bindingVersion`; replies do **not** supply lifecycle `PostStatus`, attempts, external results, safe failure summaries, publishedAt, totals or cursors. Availability filters operate on the loaded records, not a server-wide search. The verified local-record marker and an Artifact reference never imply successful publishing.

Missing capabilities remain owned by Social's published read contract: expose authorized lifecycle/attempt/external-result projections and redacted failure summaries if those user inspections are needed; define pagination/completeness if browsing beyond the bounded 200-row UTC interval is needed. These are proposals, not backend agreement. The frontend shows these facts as not provided, uses no raw diagnostics/external URL, and introduces no publishing or attempt-retry operation. Every retry button only retries an existing read. Project Open remains blocked by FB-GAP-001. No silent-revocation event or global authorization revision is supplied.

The previously observed `PostSchedulerService` JDBC `Instant` binding failure is an independent backend defect. This frontend slice neither changes the scheduler nor treats its behavior as publication success/failure evidence.

This slice's read interaction verification uses a new task-owned database, accepted backend image `22297d62b234` (872e0b08 backend), real JWT/RBAC and the three existing Social GET endpoints. It verifies authorized local records, Artifact references and backend-denied reads; it does not close Project Open, production OIDC, provider publication, attempt/outcome contracts, scheduler or silent-revocation gaps. The separate injected HTTP503 scenario verifies only frontend read-retry behavior.
