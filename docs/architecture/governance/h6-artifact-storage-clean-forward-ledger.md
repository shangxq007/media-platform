...
| `identity-access-module` | 21 | INTERNAL_STORAGE_IMPLEMENTATION_ALLOWED | Import download/write/rollback mechanics; project Artifact enumeration/export signer shadow was deleted. |
...
| Client-export Artifact compatibility port/adapter | DELETE_SHADOW | Deleted; producer commits through `ArtifactCommitService`. |
| Project-import catalog registration | DELETE_SHADOW | Raw catalog registration caller deleted; producer commits through `ArtifactCommitService`. |
...
Completed-surface arithmetic: 16 rows = 12 `DELETE_SHADOW` + 2
