...
```typescript
// 缓存用户能力信息
const cachedCapabilities = useLocalStorage('user-capabilities', null, {
  read: () => JSON.parse(localStorage.getItem('user-capabilities') || 'null'),
  write: (value) => localStorage.setItem('user-capabilities', JSON.stringify(value))
})
```
