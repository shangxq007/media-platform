...
public class TenantDataLoader implements MappedBatchLoader<String, Map<String,Object>> {
 private final TenantReadQuery query;
 public TenantDataLoader(TenantReadQuery query){this.query=query;}
 public CompletionStage<Map<String,Map<String,Object>>> load(Set<String> keys) {
  ...
  try {
   ...
   for(String id:keys) query.findById(id).ifPresent(v->result.put(id,Map.of("id",v.id(),"name",v.name(),"status",v.status())));
