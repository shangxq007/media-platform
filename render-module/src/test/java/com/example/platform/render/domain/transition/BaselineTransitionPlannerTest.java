...
/**
 * Tests for Baseline Transition Planning.
 * Covers: domain types, policy, planner, parameter validation, clip relationship,
 * determinism, safety.
 */
class BaselineTransitionPlannerTest {

    // ==================== Stage 1: Domain Types ====================

    @Test @DisplayName("Plan id rejects blank")
    void planIdRejectsBlank() {
        assertThrows(NullPointerException.class, () -> new BaselineTransitionPlanId(null));
        assertThrows(IllegalArgumentException.class, () -> new BaselineTransitionPlanId(""));
    }

    @Test @DisplayName("Operation id rejects blank")
    void operationIdRejectsBlank() {
        assertThrows(NullPointerException.class, () -> new BaselineTransitionOperationId(null));
        assertThrows(IllegalArgumentException.class, () -> new BaselineTransitionOperationId(""));
    }

    @Test @DisplayName("Plan status enum contains required statuses")
    void planStatusEnumContainsRequired() {
        assertNotNull(BaselineTransitionPlanStatus.READY);
        assertNotNull(BaselineTransitionPlanStatus.VALID_WITH_WARNINGS);
        assertNotNull(BaselineTransitionPlanStatus.INVALID);
        assertNotNull(BaselineTransitionPlanStatus.BLOCKED);
        assertNotNull(BaselineTransitionPlanStatus.UNSUPPORTED);
        assertNotNull(BaselineTransitionPlanStatus.FAILED);
    }

    @Test @DisplayName("Operation type enum contains required baseline transitions")
    void operationTypeContainsBaseline() {
        assertNotNull(BaselineTransitionOperationType.CUT);
        assertNotNull(BaselineTransitionOperationType.FADE);
        assertNotNull(BaselineTransitionOperationType.CROSSFADE);
        assertNotNull(BaselineTransitionOperationType.DISSOLVE);
    }

    @Test @DisplayName("Operation type enum contains POC transitions")
    void operationTypeContainsPoc() {
        assertNotNull(BaselineTransitionOperationType.SLIDE);
        assertNotNull(BaselineTransitionOperationType.WIPE);
        assertNotNull(BaselineTransitionOperationType.PUSH);
        assertNotNull(BaselineTransitionOperationType.ZOOM);
    }
    ...
    void operationTargetSemantic() {
        ...
        assertEquals(BaselineTransitionOperationTargetType.CLIP_PAIR, target.targetType());
    ...
    void operationParameterTyped() {
        BaselineTransitionOperationParameter param = new BaselineTransitionOperationParameter(
                "durationMs", BaselineTransitionParameterType.DURATION_MS, 500, Map.<String,String>of());
        assertEquals("durationMs", param.name());
        assertEquals(BaselineTransitionParameterType.DURATION_MS, param.type());
    ...
    void safeMetadataOnly() {
        BaselineTransitionPlan plan = new BaselineTransitionPlan(
                new BaselineTransitionPlanId("p1"),
                BaselineTransitionPlanStatus.READY,
    ...
    void requestIdRejectsBlank() {
        assertThrows(NullPointerException.class, () -> new BaselineTransitionPlanningRequestId(null));
        assertThrows(IllegalArgumentException.class, () -> new BaselineTransitionPlanningRequestId(""));
    }

    @Test @DisplayName("Request requires timeline")
    void requestRequiresTimeline() {
        assertThrows(NullPointerException.class, () ->
                new BaselineTransitionPlanningRequest(
                        new BaselineTransitionPlanningRequestId("r1"),
                        null, BaselineTransitionPolicy.conservative(), Map.of()));
    }

    @Test @DisplayName("Result supports all statuses")
    void resultSupportsStatuses() {
        assertEquals(BaselineTransitionPlanningResultStatus.PLANNED,
                BaselineTransitionPlanningResult.planned(null).status());
        assertEquals(BaselineTransitionPlanningResultStatus.BLOCKED,
                BaselineTransitionPlanningResult.blocked(List.of()).status());
        assertEquals(BaselineTransitionPlanningResultStatus.UNSUPPORTED,
                BaselineTransitionPlanningResult.unsupported(List.of()).status());
        assertEquals(BaselineTransitionPlanningResultStatus.FAILED,
                BaselineTransitionPlanningResult.failed(List.of()).status());
    }

    @Test @DisplayName("Issue severities exist")
    void issueSeveritiesExist() {
        assertNotNull(BaselineTransitionPlanIssueSeverity.INFO);
        assertNotNull(BaselineTransitionPlanIssueSeverity.WARNING);
        assertNotNull(BaselineTransitionPlanIssueSeverity.ERROR);
        assertNotNull(BaselineTransitionPlanIssueSeverity.BLOCKING);
    }

    @Test @DisplayName("Issue codes include boundary codes")
    void issueCodesIncludeBoundaries() {
        assertNotNull(BaselineTransitionPlanIssueCode.RAW_PROVIDER_EXPRESSION_FORBIDDEN);
        assertNotNull(BaselineTransitionPlanIssueCode.RAW_PROVIDER_COMMAND_FORBIDDEN);
        assertNotNull(BaselineTransitionPlanIssueCode.USER_RENDER_DAG_FORBIDDEN);
        assertNotNull(BaselineTransitionPlanIssueCode.PLUGIN_EXECUTION_NODE_FORBIDDEN);
        assertNotNull(BaselineTransitionPlanIssueCode.REMOTION_EXECUTION_FORBIDDEN);
        assertNotNull(BaselineTransitionPlanIssueCode.ARTIFACT_DAG_NOT_USED);
        assertNotNull(BaselineTransitionPlanIssueCode.RENDER_NOT_ALLOWED);
        assertNotNull(BaselineTransitionPlanIssueCode.PRODUCT_CREATION_NOT_ALLOWED);
        assertNotNull(BaselineTransitionPlanIssueCode.STORAGE_INTERNALS_FORBIDDEN);
        assertNotNull(BaselineTransitionPlanIssueCode.PROVIDER_INTERNALS_FORBIDDEN);
        assertNotNull(BaselineTransitionPlanIssueCode.PERSISTENCE_NOT_IMPLEMENTED);
        assertNotNull(BaselineTransitionPlanIssueCode.NON_ADJACENT_CLIPS);
        assertNotNull(BaselineTransitionPlanIssueCode.OVERLAP_REQUIRED_BUT_MISSING);
        assertNotNull(BaselineTransitionPlanIssueCode.INSUFFICIENT_CLIP_DURATION);
    }
    ...
    void defaultPolicyConservative() {
        BaselineTransitionPolicy policy = BaselineTransitionPolicy.conservative();
    ...
    void pocNotAllowedByDefault() {
        assertFalse(BaselineTransitionPolicy.conservative().allowPocTransitions());
    }

    @Test @DisplayName("Restricted transitions not allowed by default")
    void restrictedNotAllowedByDefault() {
        assertFalse(BaselineTransitionPolicy.conservative().allowRestrictedTransitions());
    }

    @Test @DisplayName("CUT zero duration allowed by default")
    void cutZeroDurationAllowedByDefault() {
        assertTrue(BaselineTransitionPolicy.conservative().allowCutWithZeroDuration());
    }

    @Test @DisplayName("Permissive policy allows POC")
    void permissivePolicyAllowsPoc() {
        BaselineTransitionPolicy policy = BaselineTransitionPolicy.permissive();
    ...
    void emptyTimelineReady() {
        ...
        BaselineTransitionPlanningResult result = plan(timeline);
        assertEquals(BaselineTransitionPlanningResultStatus.PLANNED, result.status());
        assertEquals(0, result.plan().operations().size());
        assertEquals(BaselineTransitionPlanStatus.READY, result.plan().status());
    }
    ...
    void adjacentClipsProduceCut() {
        ...
        assertTrue(result.plan().operations().stream()
                .anyMatch(op -> op.type() == BaselineTransitionOperationType.CUT));
    }
    ...
    void fadeTransitionProducesOperation() {
        ...
        BaselineTransitionPlanningResult result = plan(timeline);
        assertTrue(result.plan().operations().stream()
                .anyMatch(op -> op.type() == BaselineTransitionOperationType.FADE));
    }
    ...
    void dissolveTransitionProducesOperation() {
        ...
        BaselineTransitionPlanningResult result = plan(timeline);
        assertTrue(result.plan().operations().stream()
                .anyMatch(op -> op.type() == BaselineTransitionOperationType.DISSOLVE));
    }
    ...
    void pocTransitionUnsupportedByDefault() {
        ...
        assertTrue(result.plan().issues().stream().anyMatch(i ->
                i.code() == BaselineTransitionPlanIssueCode.TRANSITION_CAPABILITY_POC_ONLY));
    }

    @Test @DisplayName("POC transition allowed under permissive policy")
    void pocTransitionAllowedUnderPermissive() {
        TimelineSpec timeline = buildTimelineWithTransition("SLIDE", 500);
        BaselineTransitionPlanningResult result = planWithPolicy(timeline, BaselineTransitionPolicy.permissive());
        assertTrue(result.plan().operations().stream()
                .anyMatch(op -> op.type() == BaselineTransitionOperationType.SLIDE));
    }
    ...
    void shaderTransitionBlocked() {
        ...
        assertTrue(result.issues().stream().anyMatch(i ->
                i.code() == BaselineTransitionPlanIssueCode.TRANSITION_CAPABILITY_FORBIDDEN));
    }
    ...
    void fadeRequiresPositiveDuration() {
        ...
        BaselineTransitionPlanningResult result = plan(timeline);
        assertTrue(result.issues().stream().anyMatch(i ->
                i.code() == BaselineTransitionPlanIssueCode.INVALID_TRANSITION_DURATION));
    }
    ...
    void providerExpressionParameterRejected() {
        ...
        TimelineClipEffect effect = TimelineClipEffect.ofKey("FADE",
                Map.of("durationMs", 500, "filter", "provider_expression:v"));
        ...
        BaselineTransitionPlanningResult result = plan(timeline);
        assertTrue(result.issues().stream().anyMatch(i ->
                i.code() == BaselineTransitionPlanIssueCode.RAW_PROVIDER_EXPRESSION_FORBIDDEN));
    }
    ...
    void deterministicOrdering() {
        ...
        BaselineTransitionPlanningResult r1 = plan(timeline);
        BaselineTransitionPlanningResult r2 = plan(timeline);
    ...
    void noProviderRuntimeCall() {
        assertNotNull(BaselineTransitionPlanner.class);
    }
    ...
    void transitionTargetReferencesClips() {
        TimelineSpec timeline = buildTimelineWithTransition("FADE", 500);
        BaselineTransitionPlanningResult result = plan(timeline);
        BaselineTransitionOperation op = result.plan().operations().stream()
                .filter(o -> o.type() == BaselineTransitionOperationType.FADE)
    ...
    private BaselineTransitionPlanningResult plan(TimelineSpec timeline) {
        return BaselineTransitionPlanner.plan(new BaselineTransitionPlanningRequest(
                new BaselineTransitionPlanningRequestId("req-" + System.nanoTime()),
                timeline, BaselineTransitionPolicy.conservative(), Map.of()));
    }

    private BaselineTransitionPlanningResult planWithPolicy(TimelineSpec timeline, BaselineTransitionPolicy policy) {
        return BaselineTransitionPlanner.plan(new BaselineTransitionPlanningRequest(
                new BaselineTransitionPlanningRequestId("req-" + System.nanoTime()),
