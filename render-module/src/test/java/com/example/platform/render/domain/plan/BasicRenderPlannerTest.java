...
/**
 * Tests for Basic Timeline Render Planner.
 * Covers: domain types, policy, planner, output profile validation,
 * caption/watermark overlay validation, determinism, safety.
 */
class BasicRenderPlannerTest {

    // ==================== Stage 1: Domain Types ====================

    @Test @DisplayName("Plan id rejects blank")
    void planIdRejectsBlank() {
        assertThrows(NullPointerException.class, () -> new BasicRenderPlanId(null));
        assertThrows(IllegalArgumentException.class, () -> new BasicRenderPlanId(""));
    }

    @Test @DisplayName("Stage id rejects blank")
    void stageIdRejectsBlank() {
        assertThrows(NullPointerException.class, () -> new BasicRenderStageId(null));
        assertThrows(IllegalArgumentException.class, () -> new BasicRenderStageId(""));
    }

    @Test @DisplayName("Step id rejects blank")
    void stepIdRejectsBlank() {
        assertThrows(NullPointerException.class, () -> new BasicRenderStepId(null));
        assertThrows(IllegalArgumentException.class, () -> new BasicRenderStepId(""));
    }

    @Test @DisplayName("Plan status enum contains required statuses")
    void planStatusEnumContainsRequired() {
        assertNotNull(BasicRenderPlanStatus.READY);
        assertNotNull(BasicRenderPlanStatus.VALID_WITH_WARNINGS);
        assertNotNull(BasicRenderPlanStatus.INVALID);
        assertNotNull(BasicRenderPlanStatus.BLOCKED);
        assertNotNull(BasicRenderPlanStatus.UNSUPPORTED);
        assertNotNull(BasicRenderPlanStatus.FAILED);
    }

    @Test @DisplayName("Stage type enum contains required stage types")
    void stageTypeContainsRequired() {
        assertNotNull(BasicRenderStageType.VALIDATE_TIMELINE);
        assertNotNull(BasicRenderStageType.PREPARE_INPUTS);
        assertNotNull(BasicRenderStageType.PLAN_CLIP_SEQUENCE);
        assertNotNull(BasicRenderStageType.PLAN_EFFECTS);
        assertNotNull(BasicRenderStageType.PLAN_TRANSITIONS);
        assertNotNull(BasicRenderStageType.PLAN_CAPTION_OVERLAYS);
        assertNotNull(BasicRenderStageType.PLAN_WATERMARK_OVERLAYS);
        assertNotNull(BasicRenderStageType.PLAN_FINAL_ASSEMBLY);
        assertNotNull(BasicRenderStageType.PLAN_OUTPUT_ENCODING);
        assertNotNull(BasicRenderStageType.PLAN_OUTPUT_VERIFICATION);
    }

    @Test @DisplayName("Step type enum contains required step types")
    void stepTypeContainsRequired() {
        assertNotNull(BasicRenderStepType.VALIDATE_TIMELINE);
        assertNotNull(BasicRenderStepType.DECLARE_INPUT_CLIP);
        assertNotNull(BasicRenderStepType.DECLARE_OUTPUT_PROFILE);
        assertNotNull(BasicRenderStepType.APPLY_EFFECT_OPERATION);
        assertNotNull(BasicRenderStepType.APPLY_TRANSITION_OPERATION);
        assertNotNull(BasicRenderStepType.APPLY_CAPTION_OVERLAY);
        assertNotNull(BasicRenderStepType.APPLY_WATERMARK_OVERLAY);
        assertNotNull(BasicRenderStepType.ASSEMBLE_CLIP_SEQUENCE);
        assertNotNull(BasicRenderStepType.ENCODE_OUTPUT);
        assertNotNull(BasicRenderStepType.VERIFY_OUTPUT);
    }

    @Test @DisplayName("Step target is semantic only")
    void stepTargetSemantic() {
        BasicRenderStepTarget target = new BasicRenderStepTarget(
                BasicRenderStepTargetType.CLIP, "clip-1", Map.<String,String>of());
        assertEquals(BasicRenderStepTargetType.CLIP, target.targetType());
    ...
    void stepParameterTyped() {
        BasicRenderStepParameter param = new BasicRenderStepParameter(
                "width", BasicRenderStepParameterType.INTEGER, 1920, Map.<String,String>of());
        assertEquals("width", param.name());
        assertEquals(BasicRenderStepParameterType.INTEGER, param.type());
    ...
    void safeMetadataOnly() {
        BasicRenderPlan plan = new BasicRenderPlan(
                new BasicRenderPlanId("p1"),
                BasicRenderPlanStatus.READY,
    ...
    void requestIdRejectsBlank() {
        assertThrows(NullPointerException.class, () -> new BasicRenderPlanningRequestId(null));
        assertThrows(IllegalArgumentException.class, () -> new BasicRenderPlanningRequestId(""));
    }
    ...
    void requestRequiresTimeline() {
        assertThrows(NullPointerException.class, () ->
                new BasicRenderPlanningRequest(
                        new BasicRenderPlanningRequestId("r1"),
                        null, BasicRenderPolicy.conservative(), Map.of()));
    }

    @Test @DisplayName("Result supports all statuses")
    void resultSupportsStatuses() {
        assertEquals(BasicRenderPlanningResultStatus.PLANNED,
                BasicRenderPlanningResult.planned(null).status());
        assertEquals(BasicRenderPlanningResultStatus.BLOCKED,
                BasicRenderPlanningResult.blocked(List.of()).status());
        assertEquals(BasicRenderPlanningResultStatus.UNSUPPORTED,
                BasicRenderPlanningResult.unsupported(List.of()).status());
        assertEquals(BasicRenderPlanningResultStatus.FAILED,
                BasicRenderPlanningResult.failed(List.of()).status());
    }

    @Test @DisplayName("Issue severities exist")
    void issueSeveritiesExist() {
        assertNotNull(BasicRenderPlanIssueSeverity.INFO);
        assertNotNull(BasicRenderPlanIssueSeverity.WARNING);
        assertNotNull(BasicRenderPlanIssueSeverity.ERROR);
        assertNotNull(BasicRenderPlanIssueSeverity.BLOCKING);
    }

    @Test @DisplayName("Issue codes include boundary codes")
    void issueCodesIncludeBoundaries() {
        assertNotNull(BasicRenderPlanIssueCode.RAW_PROVIDER_EXPRESSION_FORBIDDEN);
        assertNotNull(BasicRenderPlanIssueCode.RAW_PROVIDER_COMMAND_FORBIDDEN);
        assertNotNull(BasicRenderPlanIssueCode.USER_RENDER_DAG_FORBIDDEN);
        assertNotNull(BasicRenderPlanIssueCode.PLUGIN_EXECUTION_NODE_FORBIDDEN);
        assertNotNull(BasicRenderPlanIssueCode.REMOTION_EXECUTION_FORBIDDEN);
        assertNotNull(BasicRenderPlanIssueCode.ARTIFACT_DAG_NOT_USED);
        assertNotNull(BasicRenderPlanIssueCode.OPEN_CUE_NOT_USED);
        assertNotNull(BasicRenderPlanIssueCode.RENDER_EXECUTION_NOT_ALLOWED);
        assertNotNull(BasicRenderPlanIssueCode.PRODUCT_CREATION_NOT_ALLOWED);
        assertNotNull(BasicRenderPlanIssueCode.STORAGE_RUNTIME_FORBIDDEN);
        assertNotNull(BasicRenderPlanIssueCode.PRODUCT_RUNTIME_FORBIDDEN);
        assertNotNull(BasicRenderPlanIssueCode.PROVIDER_INTERNALS_FORBIDDEN);
        assertNotNull(BasicRenderPlanIssueCode.PERSISTENCE_NOT_IMPLEMENTED);
        assertNotNull(BasicRenderPlanIssueCode.PUBLIC_API_NOT_IMPLEMENTED);
    }
    ...
    void defaultPolicyConservative() {
        BasicRenderPolicy policy = BasicRenderPolicy.conservative();
    ...
    void emptyValidTimelineProducesPlannedPlan() {
        ...
        BasicRenderPlanningResult result = plan(timeline);
        assertEquals(BasicRenderPlanningResultStatus.PLANNED, result.status());
        assertNotNull(result.plan());
        assertEquals(BasicRenderPlanStatus.READY, result.plan().status());
    }
    ...
    void validEffectPlanProducesSteps() {
        ...
        assertTrue(result.plan().stages().stream()
                ...
                .anyMatch(s -> s.type() == BasicRenderStepType.APPLY_EFFECT_OPERATION));
    }
    ...
    void validTransitionPlanProducesSteps() {
        ...
        assertTrue(result.plan().stages().stream()
                .flatMap(s -> s.steps().stream())
                .anyMatch(s -> s.type() == BasicRenderStepType.APPLY_TRANSITION_OPERATION));
    }
    ...
    void clipSequenceProducesSteps() {
        ...
        assertTrue(result.plan().stages().stream()
                .flatMap(s -> s.steps().stream())
                .anyMatch(s -> s.type() == BasicRenderStepType.DECLARE_INPUT_CLIP));
        assertTrue(result.plan().stages().stream()
                .flatMap(s -> s.steps().stream())
                .anyMatch(s -> s.type() == BasicRenderStepType.ASSEMBLE_CLIP_SEQUENCE));
    }
    ...
    void outputProfileProducesSteps() {
        ...
        assertTrue(result.plan().stages().stream()
                .flatMap(s -> s.steps().stream())
                .anyMatch(s -> s.type() == BasicRenderStepType.DECLARE_OUTPUT_PROFILE));
        assertTrue(result.plan().stages().stream()
                .flatMap(s -> s.steps().stream())
                .anyMatch(s -> s.type() == BasicRenderStepType.ENCODE_OUTPUT));
    }
    ...
    void verificationStepAlwaysPresent() {
        ...
        assertTrue(result.plan().stages().stream()
                .flatMap(s -> s.steps().stream())
                .anyMatch(s -> s.type() == BasicRenderStepType.VERIFY_OUTPUT));
    }

    @Test @DisplayName("No RenderJob/Product/StorageRuntime/OpenCue calls")
    void noExternalCalls() {
        assertNotNull(BasicRenderPlanner.class);
    }
    ...
    void mp4ContainerAccepted() {
        ...
        assertFalse(result.issues().stream().anyMatch(i ->
                i.code() == BasicRenderPlanIssueCode.UNSUPPORTED_OUTPUT_CONTAINER));
    }
    ...
    void h264VideoCodecAccepted() {
        ...
        BasicRenderPlanningResult result = plan(timeline);
        assertFalse(result.issues().stream().anyMatch(i ->
                i.code() == BasicRenderPlanIssueCode.UNSUPPORTED_VIDEO_CODEC));
    }
    ...
    void captionOverlayRequiresText() {
        ...
        BasicRenderPlanningResult result = plan(timeline);
        assertTrue(result.issues().stream().anyMatch(i ->
                i.code() == BasicRenderPlanIssueCode.CAPTION_OVERLAY_INVALID));
    }
    ...
    void watermarkOpacityMustBeValid() {
        ...
        BasicRenderPlanningResult result = plan(withWatermark);
        assertTrue(result.issues().stream().anyMatch(i ->
                i.code() == BasicRenderPlanIssueCode.WATERMARK_OVERLAY_INVALID));
    }
    ...
    void stageOrderingDeterministic() {
        TimelineSpec timeline = TimelineSpec.create("tl-1", "Test", TimelineOutputSpec.mp4_1080p30());
        BasicRenderPlanningResult r1 = plan(timeline);
        BasicRenderPlanningResult r2 = plan(timeline);
    ...
    void stepOrderingDeterministic() {
        ...
        BasicRenderPlanningResult r1 = plan(timeline);
        BasicRenderPlanningResult r2 = plan(timeline);
        List<BasicRenderStep> steps1 = r1.plan().stages().stream()
                .flatMap(s -> s.steps().stream()).toList();
        List<BasicRenderStep> steps2 = r2.plan().stages().stream()
                .flatMap(s -> s.steps().stream()).toList();
    ...
    private BasicRenderPlanningResult plan(TimelineSpec timeline) {
        return BasicRenderPlanner.plan(new BasicRenderPlanningRequest(
                new BasicRenderPlanningRequestId("req-" + System.nanoTime()),
                timeline, BasicRenderPolicy.conservative(), Map.of()));
    }
