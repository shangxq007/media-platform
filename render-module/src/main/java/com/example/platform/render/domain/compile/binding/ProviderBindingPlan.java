package com.example.platform.render.domain.compile.binding;
...
public record ProviderBindingPlan(
    ...
    public ProviderBindingNode finalRenderNode() {
        return nodes.stream()
                .filter(n -> n.artifactNodeType() == com.example.platform.render.domain.compile.ArtifactNodeType.FINAL_RENDER)
                .findFirst()
                .orElse(null);
    }
