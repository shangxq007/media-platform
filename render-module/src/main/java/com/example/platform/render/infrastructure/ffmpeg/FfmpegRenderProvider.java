 ...
 *
 * <p>This provider executes render jobs using FFmpeg. It is activated only when
 * the {@code render.providers.ffmpeg.enabled} property is set to {@code true}.</p>
 *
 * <p>All process execution goes through the {@link ProcessToolRunner} port — no
 * direct {@link ProcessBuilder} or shell string concatenation is used.</p>
 *
 * <h3>Activation</h3>
 * <pre>
 * render:
...
public class FfmpegRenderProvider implements RenderProvider {
    ...
    public RenderResult render(String jobId, String aiScript, String profile) {
        ...
        // Skeleton: build a transcode command from the profile
        // In a full implementation, the aiScript would be parsed to determine
        // input sources, filters, and output parameters
        RenderProfile renderProfile = RenderProfile.of(profile, "1920x1080", "h264");

        // This is a placeholder — actual implementation would parse the aiScript
