...
public class RenderJobSubmissionService {
    ...
    private void applyAiEditInstructionIfPresent(String jobId, SubmitRenderJobRequest request) {
        ...
        if (request.targetSegmentIds() != null && !request.targetSegmentIds().isEmpty()) {
            try {
                var node = com.example.platform.timeline.api.serialization.InternalTimelineJson.parse(timelineJson);
                if (node.isObject()) {
                    ...
                    timelineJson = com.example.platform.timeline.api.serialization.InternalTimelineJson.write(doc);
                }
            } catch (Exception e) {
