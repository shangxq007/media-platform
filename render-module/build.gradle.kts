...
tasks.register("verifyC20RenderPlanBoundaryGuard") {
    ...
    doLast {
        ...
        require(migrationDir.listFiles().orEmpty().filter { it.name.endsWith(".sql") }.map { it.name }.toSet()
                == setOf("V1__initial_schema.sql", "V2__account_membership_and_project_scope.sql", "V3__quarantine_unresolved_assignment_scopes.sql", "V4__quarantine_missing_assignment_tenants.sql", "V5__workflow_durable_runs.sql", "V6__marketplace_owner.sql")) {
