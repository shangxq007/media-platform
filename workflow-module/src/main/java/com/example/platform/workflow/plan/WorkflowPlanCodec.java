...
public final class WorkflowPlanCodec {
    ...
    public WorkflowPlan decode(String json) {
        try {
            ...
            throw new IllegalArgumentException("Invalid typed Workflow plan", e);
        }
    }
            ...
            var ordered =
                    new WorkflowPlan(
                            ...
                            plan.nodes().stream()
                                    ...
                                    .sorted(Comparator.comparing(WorkflowPlan.Node::id))
                                    .toList(),
                            plan.edges().stream()
                                    .sorted(
                                            Comparator.comparing(WorkflowPlan.ControlEdge::parentId)
                                                    .thenComparingInt(
                                                            WorkflowPlan.ControlEdge::order))
                                    .toList());
            return mapper.writeValueAsString(ordered);
        } catch (java.io.IOException e) {
            throw new IllegalArgumentException("Unserializable Workflow plan", e);
        }
    }
    ...
    private WorkflowPlan.Node normalizeNode(WorkflowPlan.Node node) {
        ...
        if (child != null)
            child = new WorkflowPlan.ChildPin(child.digest(), decode(encode(child.plan())));
        return new WorkflowPlan.Node(
                node.id(),
                node.kind(),
                node.predicate(),
                node.waitSpec(),
