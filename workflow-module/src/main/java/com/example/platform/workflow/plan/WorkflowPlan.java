...
public record WorkflowPlan(
    ...
    public WorkflowPlan {
        if (formatVersion != 1 || definitionVersion < 1)
    ...
    public record Predicate(Comparison comparison, ValueRef left, String expectedJson) {
        public Predicate {
            ...
            if ((comparison == Comparison.EQUAL || comparison == Comparison.NOT_EQUAL)
                    != (expectedJson != null))
                throw new IllegalArgumentException("Comparison operand mismatch");
        }
    }

    /**
     * Child ownership is explicit. Ordered child edges describe structured control, never data
     * flow.
     */
    ...
    public record Wait(WaitKind kind, long timeoutMillis) {
        public Wait {
            ...
            if (timeoutMillis < 1) throw new IllegalArgumentException("Wait deadline required");
        }
    }

    public record Retry(int maximumAttempts, long initialDelayMillis) {
        public Retry {
            if (maximumAttempts < 1 || initialDelayMillis < 1)
                throw new IllegalArgumentException("Bounded retry required");
        }
    }
    ...
    public record Node(
        ...
        public static Node control(String id, Kind kind) {
            return new Node(
                    id, kind, null, null, 0, 0, null, null, List.of(), Map.of(), null, null);
        }
    }

    private static void text(String text) {
        if (text == null || text.isBlank())
            throw new IllegalArgumentException("Required plan identity");
    }
}
