...
/** The single argv-only external container-engine process boundary. */
final class ContainerEngineProcess {
    private static final int DIAGNOSTIC_LIMIT = 16 * 1024;

    private ContainerEngineProcess() {}
    ...
    static EngineResult execute(
        ...
        try {
            ...
            return new EngineResult(processCompleted ? process.exitValue() : -1,
                    stdout.text(), stderr.text(), processCompleted && capturesClosed);
        } catch (IOException failure) {
            return new EngineResult(-1, "", failure.getClass().getSimpleName(), false);
        } catch (InterruptedException interrupted) {
            Thread.currentThread().interrupt();
            return new EngineResult(-1, "", "interrupted", false);
        } finally {
    ...
    record EngineResult(int exitCode, String stdout, String stderr, boolean completed) {
        boolean succeeded() { return completed && exitCode == 0; }
        String diagnostic() {
    ...
    private static final class DiagnosticReader implements Runnable, AutoCloseable {
        ...
        private String text() { return captured.toString(StandardCharsets.UTF_8); }
