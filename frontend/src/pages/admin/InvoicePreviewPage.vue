...
function statusClass(status: string): string {
  switch (status) {
    ...
    case 'ISSUED': return 'bg-info-muted text-info'
    case 'DRAFT': return 'bg-yellow-600/20 text-warning'
    case 'OVERDUE': return 'bg-danger-muted text-danger'
    case 'VOID': return 'bg-surface-4/20 text-text-secondary'
...
<template>
  <div class="flex-1 overflow-y-auto p-6 space-y-6">
    <div class="flex items-center justify-between">
      ...
      <button class="px-3 py-1.5 bg-surface-3 hover:bg-surface-4 text-sm rounded text-white" @click="loadInvoices">Refresh</button>
    </div>

    <!-- Preview Generator -->
    <div class="bg-surface-2 border border-border-subtle rounded-lg p-4 space-y-3">
      <h3 class="text-sm font-semibold text-text-primary">Preview Invoice</h3>
      <div class="flex gap-2">
        <input v-model="previewTenantId" placeholder="Tenant ID" class="flex-1 bg-surface-3 border border-border-default rounded px-2 py-1.5 text-sm text-white" />
      ...
      <div v-if="preview" class="p-3 rounded bg-surface-3/30 border border-border-default">
    ...
    <!-- Filter -->
    <div class="flex gap-2">
      <select v-model="filterStatus" class="bg-surface-2 border border-border-default rounded px-2 py-1.5 text-sm text-white" @change="loadInvoices">
    ...
    <div v-else-if="invoices.length === 0" class="text-text-tertiary text-sm">No invoices</div>
    <div v-else class="space-y-3">
      <div v-for="inv in invoices" :key="inv.invoiceId" class="bg-surface-2 border border-border-subtle rounded-lg p-4">
        <div class="flex items-center justify-between mb-2">
        ...
        <div class="flex gap-4 text-xs text-text-tertiary mb-2">
          <span>Issued: {{ inv.issuedAt }}</span>
          <span>Due: {{ inv.dueAt }}</span>
          <span v-if="inv.paidAt">Paid: {{ inv.paidAt }}</span>
        </div>
        <div class="flex gap-2">
          <button v-if="inv.status === 'DRAFT'" class="text-[10px] text-info hover:text-info" @click="issueInvoice(inv.invoiceId)">Issue</button>
          <button v-if="inv.status !== 'VOID' && inv.status !== 'PAID'" class="text-[10px] text-danger hover:text-danger" @click="voidInvoice(inv.invoiceId)">Void</button>
        </div>
      </div>
    </div>
  </div>
