...
<template>
  <div class="flex-1 overflow-y-auto p-6">
    ...
    <!-- Publish Form -->
    <div v-if="showPublish" class="bg-gray-800 border border-gray-700 rounded-lg p-4 mb-6">
      <h3 class="text-xs font-semibold text-gray-300 mb-3">Publish Notification Event</h3>
      <div class="flex gap-3 max-w-lg">
        <input v-model="publishType" type="text" class="flex-1 bg-gray-700 border border-gray-600 rounded px-2 py-1.5 text-sm text-white" placeholder="Event type" />
        <input v-model="publishPayload" type="text" class="flex-1 bg-gray-700 border border-gray-600 rounded px-2 py-1.5 text-sm text-white font-mono" placeholder='{"key":"value"}' />
        <button class="px-3 py-1.5 bg-blue-600 hover:bg-blue-500 text-sm rounded" @click="publishEvent">Send</button>
      </div>
    </div>

    <div v-if="loading" class="text-gray-400 text-sm">Loading...</div>
    <div v-else-if="notifications.length === 0" class="text-gray-500 text-sm">No notifications</div>
      ...
      >
        <div class="flex items-center justify-between">
          <div>
            ...
            <span class="text-xs text-gray-500 ml-2 font-mono">{{ n.id }}</span>
          </div>
          <div class="flex items-center gap-2">
            <span class="text-xs px-1.5 py-0.5 rounded bg-gray-700 text-gray-300">{{ n.status }}</span>
            <button class="text-xs px-2 py-0.5 bg-gray-700 hover:bg-gray-600 rounded" @click="viewDeliveries(n.id!)">Deliveries</button>
            <button class="text-xs px-2 py-0.5 bg-blue-600/20 text-blue-300 rounded" @click="retryNotification(n.id!)">Retry</button>
          </div>
        </div>
        <div class="text-xs text-gray-500 mt-1">{{ n.createdAt }}</div>

        <!-- Deliveries -->
        <div v-if="selectedId === n.id && deliveries.length > 0" class="mt-3 pt-3 border-t border-gray-700 space-y-1">
          <div v-for="d in deliveries" :key="d.id" class="flex items-center gap-3 text-xs">
            <span class="px-1.5 py-0.5 rounded bg-gray-700">{{ d.channel }}</span>
            <span :class="d.status === 'DELIVERED' ? 'text-green-400' : 'text-red-400'">{{ d.status }}</span>
            <span class="text-gray-500">{{ d.sentAt || '—' }}</span>
          </div>
        </div>
      </div>
    </div>
