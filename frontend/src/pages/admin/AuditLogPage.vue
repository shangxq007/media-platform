...
function categoryClass(category?: string): string {
  if (!category) return 'bg-gray-600/20 text-gray-400'
  if (category.startsWith('FEATURE_FLAG_')) return 'bg-purple-600/20 text-purple-300'
  if (category.startsWith('POLICY_')) return 'bg-blue-600/20 text-blue-300'
  if (category.startsWith('ACCESS_')) return 'bg-green-600/20 text-green-300'
  ...
  <div class="flex-1 overflow-y-auto p-6">
    <div class="flex items-center justify-between mb-6">
      <div>
        ...
        <p class="text-sm text-gray-400 mt-1">Comprehensive audit trail with sensitive field masking</p>
      </div>
      <button class="px-3 py-1.5 bg-gray-700 hover:bg-gray-600 text-sm rounded text-white" @click="loadRecords">
        Refresh
      </button>
    </div>
    ...
    <div class="bg-gray-800 border border-gray-700 rounded-lg p-4 mb-4">
      ...
      <div class="grid grid-cols-5 gap-3">
        <div>
          ...
          <select v-model="filterCategory" class="w-full bg-gray-900 border border-gray-700 rounded px-2 py-1.5 text-sm text-gray-200">
        ...
        <div>
          ...
          <input v-model="filterTenant" type="text" class="w-full bg-gray-900 border border-gray-700 rounded px-2 py-1.5 text-sm text-gray-200" placeholder="Filter by tenant" />
        </div>
        <div>
          <label class="block text-xs text-gray-400 mb-1">Workspace</label>
          <input v-model="filterWorkspace" type="text" class="w-full bg-gray-900 border border-gray-700 rounded px-2 py-1.5 text-sm text-gray-200" placeholder="Filter by workspace" />
        </div>
        <div>
          <label class="block text-xs text-gray-400 mb-1">User</label>
          <input v-model="filterUser" type="text" class="w-full bg-gray-900 border border-gray-700 rounded px-2 py-1.5 text-sm text-gray-200" placeholder="Filter by user" />
        </div>
        <div>
          <label class="block text-xs text-gray-400 mb-1">Actor</label>
          <input v-model="filterActor" type="text" class="w-full bg-gray-900 border border-gray-700 rounded px-2 py-1.5 text-sm text-gray-200" placeholder="Filter by actor" />
        </div>
      </div>
    </div>
    ...
    <template v-else>
            ...
            <template v-for="record in filteredRecords" :key="record.id || Math.random()">
              ...
              >
                <td class="px-3 py-2 text-center">
                  <span class="text-xs text-gray-500">{{ expandedRecord === record.id ? '▼' : '▶' }}</span>
                </td>
                <td class="px-3 py-2 text-xs text-gray-400 whitespace-nowrap">{{ record.createdAt?.slice(0, 19) || '—' }}</td>
                <td class="px-3 py-2">
                  <span class="text-xs px-1.5 py-0.5 rounded" :class="categoryClass(record.category)">
                    {{ record.category || '—' }}
                  </span>
                </td>
                <td class="px-3 py-2 text-xs text-gray-200">{{ record.action || '—' }}</td>
                <td class="px-3 py-2 text-xs font-mono text-gray-400">{{ record.resourceDisplay }}</td>
                <td class="px-3 py-2 text-xs text-gray-400">{{ record.actor || '—' }}</td>
                <td class="px-3 py-2 text-xs text-gray-500 truncate max-w-xs">{{ maskSensitive(record.details || '—') }}</td>
              </tr>
              <tr v-if="expandedRecord === record.id" class="bg-gray-900/50">
                <td colspan="7" class="px-6 py-3">
                  <div class="grid grid-cols-2 gap-4 text-xs">
                    <div>
                      <div class="text-gray-400 mb-1">Full Details</div>
                      <pre class="text-gray-300 bg-gray-800 rounded p-2 overflow-x-auto max-h-32">{{ record.details || '—' }}</pre>
                    </div>
                    <div>
                      <div class="text-gray-400 mb-1">Before / After</div>
                      <div v-if="record.beforeJson" class="mb-2">
                        <span class="text-gray-500">Before:</span>
                        <pre class="text-red-300 bg-gray-800 rounded p-2 overflow-x-auto max-h-24 mt-1">{{ record.beforeJson }}</pre>
                      </div>
                      <div v-if="record.afterJson">
                        <span class="text-gray-500">After:</span>
                        <pre class="text-green-300 bg-gray-800 rounded p-2 overflow-x-auto max-h-24 mt-1">{{ record.afterJson }}</pre>
                      </div>
                      <div v-if="record.matchedRule" class="mt-2">
                        <span class="text-gray-500">Matched Rule:</span>
                        <span class="text-blue-300 ml-1">{{ record.matchedRule }}</span>
                      </div>
                    </div>
                  </div>
                </td>
      ...
      <div class="flex items-center justify-between mt-4">
        <span class="text-xs text-gray-500">{{ total }} total records</span>
