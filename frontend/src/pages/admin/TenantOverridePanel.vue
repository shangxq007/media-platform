...
function typeClass(type: string): string {
  switch (type) {
    case 'GRANT': return 'bg-green-600/20 text-green-400'
    case 'DENY': return 'bg-red-600/20 text-red-400'
    case 'QUOTA': return 'bg-yellow-600/20 text-yellow-400'
...
<template>
  <div class="flex-1 overflow-y-auto p-6 space-y-6">
    ...
    <template v-else>
      <div v-if="showCreate" class="bg-gray-800 border border-gray-700 rounded-lg p-4 space-y-3">
        <div class="grid grid-cols-2 gap-3">
          <div>
            ...
            <input v-model="form.tenantId" class="w-full bg-gray-700 border border-gray-600 rounded px-2 py-1.5 text-sm text-white" />
          </div>
          <div>
        ...
        <div class="grid grid-cols-2 gap-3">
          <div>
            ...
            <select v-model="form.overrideType" class="w-full bg-gray-700 border border-gray-600 rounded px-2 py-1.5 text-sm text-white">
          ...
          <div v-if="form.overrideType === 'QUOTA'">
            <label class="text-xs text-gray-400 block mb-1">Quota Value</label>
            <input v-model.number="form.quotaValue" type="number" class="w-full bg-gray-700 border border-gray-600 rounded px-2 py-1.5 text-sm text-white" />
          </div>
        </div>
        <div>
      ...
      <div v-if="overrides.length === 0" class="text-gray-500 text-sm">No tenant overrides</div>
      <div v-else class="bg-gray-800 border border-gray-700 rounded-lg overflow-hidden">
        <table class="w-full text-sm">
          ...
          <tbody>
            <tr v-for="ov in overrides" :key="ov.overrideId" class="border-b border-gray-700/50">
              <td class="px-4 py-2 text-xs text-white font-mono">{{ ov.tenantId }}</td>
              <td class="px-4 py-2 text-xs text-gray-300">{{ ov.featureKey }}</td>
              <td class="px-4 py-2"><span class="px-1.5 py-0.5 rounded text-[10px]" :class="typeClass(ov.overrideType)">{{ ov.overrideType }}</span></td>
              <td class="px-4 py-2 text-xs text-gray-400">{{ ov.reason }}</td>
              <td class="px-4 py-2 text-xs text-gray-500">{{ ov.expiresAt || '—' }}</td>
              <td class="px-4 py-2"><button class="text-[10px] text-red-400 hover:text-red-300" @click="deleteOverride(ov.overrideId)">Delete</button></td>
            </tr>
          </tbody>
        </table>
      </div>
